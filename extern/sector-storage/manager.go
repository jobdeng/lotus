...
func (m *Manager) SealPreCommit1(ctx context.Context, sector storage.SectorRef, ticket abi.SealRandomness, pieces []abi.PieceInfo) (out storage.PreCommit1Out, err error) {
	...
	// PreCommit1 should be doing on local machine that has the unsealed data, and alloc space enough.
	selector := newAllocExistingSelector(m.index, sector.ID, storiface.FTCache|storiface.FTSealed|storiface.FTUnsealed, storiface.PathSealing)
...
func (m *Manager) SealPreCommit2(ctx context.Context, sector storage.SectorRef, phase1Out storage.PreCommit1Out) (out storage.SectorCids, err error) {
	...
	selector := newFixSelector(m.index, sector.ID, storiface.FTCache|storiface.FTSealed, storiface.PathSealing)
...
func (m *Manager) FinalizeSector(ctx context.Context, sector storage.SectorRef, keepUnsealed []storage.Range) error {
	...
	fetchSel := newAllocExistingSelector(m.index, sector.ID, storiface.FTCache|storiface.FTSealed, storiface.PathStorage)
